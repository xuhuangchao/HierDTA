...
parser.add_argument('--use_a2p', action='store_true', help='use a2p attention')
parser.add_argument('--use_m2p', action='store_true', help='use m2p attention')
parser.add_argument('--use_pmax', action='store_true', help='use protein max pooling')
...
print(f"ablation: use_a2p {args.use_a2p}, use_m2p {args.use_m2p}, use_pmax {args.use_pmax}")

name = f"wom2p_pegnn_warmup_epoch{NUM_EPOCHS}_pat{EARLY_STOPPING_PATIENCE}_norm{max_norm}"
...
for dataset in datasets:
    for seed in seeds:
        ...
        if (not os.path.isfile(processed_data_file_train)) or (not os.path.isfile(processed_data_file_val)) or (not os.path.isfile(processed_data_file_test)):
            ...
            model = modeling(use_a2p_attn=args.use_a2p, 
                             use_m2p_attn=args.use_m2p, 
                             use_protein_max_pool=args.use_pmax).to(device)
            loss_fn = nn.MSELoss()
