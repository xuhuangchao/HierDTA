...
class DualInteractionModule(nn.Module):
    def __init__(self, emb_dim, fp_dim=1024, esm_dim=480, dropout=0.2, 
        ...
        self.use_multi_level_reps = use_multi_level_reps
        ...
        if predictor_in_dim == 0:
            raise ValueError("At least one of use_multi_level_reps, use_fingerprint, or use_p_global must be True.")

        self.predictor = MLPDecoder(predictor_in_dim, 1024, 256, 1)
    ...
    def forward(self, atom_feat, motif_feat, batch_d, node_types, fingerprint, h_node_p, batch_p, esm_feat):
        ...
        if self.use_multi_level_reps:      
            ...
            attention_weights = (a2p_attn + m2p_attn)/2.0
            multi_level_reps = torch.bmm(attention_weights.unsqueeze(1), protein_rep).squeeze(1)
            attn_dict_key = ["a2p_attn", "a2p_scores", "m2p_attn", "m2p_scores"]
            attn_dict_val = [a2p_attn, a2p_scores, m2p_attn, m2p_scores]
            for key, val in zip(attn_dict_key, attn_dict_val):
                output_dict[key] = val
            
            protein_max_pooled = global_max_pool(h_node_p, batch_p)
            multi_level_reps = multi_level_reps + protein_max_pooled
        else:
            multi_level_reps = None
        
        final_feats = []
        if self.use_multi_level_reps:
            final_feats.append(multi_level_reps)
        if self.use_fingerprint:
            final_feats.append(fingerprint_emb)
        if self.use_p_global:
            final_feats.append(p_global_embedded)

        final_representation = torch.cat(final_feats, dim=1)
        logits = self.predictor(final_representation)
        
        return logits, output_dict

class DualDTA(nn.Module):
    def __init__(self, num_features_xd=47, num_features_xt=480, dropout=0.2, use_fingerprint=True, use_p_global=True, use_multi_level_reps=True): 
        super().__init__()
        ...
        self.interaction = DualInteractionModule(emb_dim=128, dropout=dropout, use_fingerprint=use_fingerprint, use_p_global=use_p_global, use_multi_level_reps=use_multi_level_reps)  # 统一为128才能这样写        
