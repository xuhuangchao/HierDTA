...
class DualInteractionModule(nn.Module):
    ...
    def forward(self, atom_feat, motif_feat, batch_d, node_types, fingerprint, h_node_p, batch_p, esm_feat):
        ...
        final_representation = torch.cat([multi_level_reps, fingerprint, p_global_embedded], dim=1)        
...
class DualDTA(nn.Module):
    def __init__(self, num_features_xd=47, num_features_xt=608, dropout=0.2): 
        super().__init__()
        ...
        self.interaction = DualInteractionModule(emb_dim=128, dropout=dropout)  # 统一为128才能这样写
